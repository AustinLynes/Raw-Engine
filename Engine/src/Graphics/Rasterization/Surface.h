...
bool InitializeSurface( _In_z_ const char* _windowTitle, _In_range_(1, 0xFFFF) unsigned int _width, _In_range_(1, 0xFFFF) unsigned int _height);

// Updates the RasterSurface with a block of raw XRGB pixel data.
// Incoming data must 32bit pixels 8 bits per channel.
bool UpdateSurface(	_In_reads_(_numPixels) const unsigned int *_xrgbPixels, 
