...
bool InitializeSurface(	_In_z_ const char* _studentName,
...
bool UpdateSurface(	_In_reads_(_numPixels) const unsigned int *_argbPixels,
...
BOOL WINAPI ConsoleCtrlHandler(DWORD ctrlCode) 
{
	...
	if (ctrlCode == CTRL_BREAK_EVENT || ctrlCode == CTRL_CLOSE_EVENT ||
		...
		ShutdownSurface(); // kill window and wait for shutdown
